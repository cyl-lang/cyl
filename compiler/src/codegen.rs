use crate::ast::{
    BinaryOperator, Expression, FunctionDeclaration,
...
impl<'ctx> LLVMCodegen<'ctx> {
    ...
    fn declare_function(&mut self, function: &FunctionDeclaration) -> Result<(), CylError> {
        let param_types: Vec<BasicMetadataTypeEnum> = function
            ...
            .iter()
            .map(|param| {
                self.cyl_type_to_llvm(&param.param_type)
                    .map(|t| t.into())
            })
        ...
        let fn_type = if let Some(ref rt) = function.return_type {
            if matches!(rt, Type::Void) {
                ...
                match return_type {
                    ...
                    BasicTypeEnum::VectorType(vector_type) => vector_type.fn_type(&param_types, false),
                }
            }
        } else {
        ...
        let param_types_cyl: Vec<Type> = function.parameters.iter().map(|p| p.param_type.clone()).collect();
        self.function_signatures.insert(function.name.clone(), (param_types_cyl, function.return_type.clone()));
    ...
    fn compile_function(&mut self, function: &FunctionDeclaration) -> Result<(), CylError> {
        ...
        for (i, param) in function.parameters.iter().enumerate() {
            ...
            self.variables.insert(param.name.clone(), (alloca, param.param_type.clone()));
        }
    ...
    fn compile_statement(&mut self, statement: &Statement) -> Result<(), CylError> {
        match statement {
            ...
            Statement::Declare(declare_stmt) => {
                ...
                let var_type = if let Some(ref explicit_type) = declare_stmt.var_type {
                    ...
                    match &declare_stmt.value {
                        Expression::Call { callee, .. } => {
                            if let Expression::Identifier(function_name) = callee.as_ref() {
                                if let Some((_, return_type)) = self.function_signatures.get(function_name) {
                ...
                self.variables.insert(declare_stmt.name.clone(), (alloca, var_type));
            }
                ...
                match operator {
                    BinaryOperator::Add => {
                        if left_val.is_int_value() && right_val.is_int_value() {
                            ...
                            Ok(self.builder.build_float_add(left_float, right_float, "addtmp").unwrap().into())
                        } else {
                            Err(CylError::CodeGenError {
                    ...
                    BinaryOperator::Subtract => {
                        if left_val.is_int_value() && right_val.is_int_value() {
                            ...
                            Err(CylError::CodeGenError {
                                message: "Subtraction not implemented for non-integers yet".to_string(),
                            })
                        }
                    }
                    BinaryOperator::Multiply => {
                        if left_val.is_int_value() && right_val.is_int_value() {
                            ...
                            Err(CylError::CodeGenError {
                                message: "Multiplication not implemented for non-integers yet".to_string(),
                            })
                        }
                    }
                    BinaryOperator::Divide => {
                        if left_val.is_int_value() && right_val.is_int_value() {
                            ...
                            Err(CylError::CodeGenError {
                                message: "Division not implemented for non-integers yet".to_string(),
                            })
                        }
                    }
                    BinaryOperator::Equal => {
                        if left_val.is_int_value() && right_val.is_int_value() {
                            ...
                            Err(CylError::CodeGenError {
                                message: "Equality comparison not implemented for non-integers yet".to_string(),
                            })
                        }
                    }
                    BinaryOperator::NotEqual => {
                        if left_val.is_int_value() && right_val.is_int_value() {
                            ...
                            Err(CylError::CodeGenError {
                                message: "Not-equal comparison not implemented for non-integers yet".to_string(),
                            })
                        }
                    }
                    BinaryOperator::Less => {
                        if left_val.is_int_value() && right_val.is_int_value() {
                            ...
                            Err(CylError::CodeGenError {
                                message: "Less-than comparison not implemented for non-integers yet".to_string(),
                            })
                        }
                    }
                    BinaryOperator::LessEqual => {
                        if left_val.is_int_value() && right_val.is_int_value() {
                            ...
                            Err(CylError::CodeGenError {
                                message: "Less-equal comparison not implemented for non-integers yet".to_string(),
                            })
                        }
                    }
                    BinaryOperator::Greater => {
                        if left_val.is_int_value() && right_val.is_int_value() {
                            ...
                            Err(CylError::CodeGenError {
                                message: "Greater-than comparison not implemented for non-integers yet".to_string(),
                            })
                        }
                    }
                    BinaryOperator::GreaterEqual => {
                        if left_val.is_int_value() && right_val.is_int_value() {
                            ...
                            Err(CylError::CodeGenError {
                                message: "Greater-equal comparison not implemented for non-integers yet".to_string(),
                            })
                        }
                    }
            ...
            Expression::Call { callee, arguments } => {
                ...
                if let Expression::Identifier(function_name) = callee.as_ref() {
                    if let Some(fn_value) = self.functions.get(function_name).copied() {
                        ...
                        let call_result = self.builder.build_call(fn_value, &args, "calltmp").unwrap();
                        if let Some(result) = call_result.try_as_basic_value().left() {
                            ...
                            Err(CylError::CodeGenError {
                                message: format!("Function '{}' returns void and cannot be used in expressions", function_name),
                            })
                        }
                    } else {
                        Err(CylError::CodeGenError {
    ...
    fn create_entry_block_alloca(&self, name: &str, cyl_type: &Type) -> Result<PointerValue<'ctx>, CylError> {
